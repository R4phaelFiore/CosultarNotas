...
while True:
    try:
        ...
        if menu == 0:
            ...
            if media >= 6:
                print(f"\nMuito bem {novoAluno} foi aprovado: Sua nota é {media}")
            else:
                print(f"\nA média das 4 notas de {novoAluno} é: {media}")
                notaRecuperacao = int(input("\nInsira a nota da prova de recuperação: "))
                mediaFinal = (media + notaRecuperacao) / 2
                
                if mediaFinal >= 6:
                    print(f"\nAluno(a) {novoAluno}, foi aprovado com media final: {mediaFinal}")
                else:
                    print(f"\nAluno(a) {novoAluno}, foi reprovado. Sua media é: {mediaFinal}")
        elif menu == 3:
            print("Saindo...")
            break
